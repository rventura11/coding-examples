...
ar.report_generated_timestamp as date
FROM project pj,
request r,
panel p,
panel_qc pq,
...
### *** duplicates_filtered with only most recent results: ***
...
write_csv(panel_qc_final, glue("PanelQC_historical_data_raw_{report_date}.csv"))
...
write_csv(graph_data, glue("PanelQC_historical_PWD_data_{report_date}.csv"))
...
graph_data <- read_csv(glue("PanelQC_historical_PWD_data_{report_date}.csv"))
...
ggsave(glue("PanelQC_historical_PWD_graph_{report_date}.png"), plot = bar_graph)